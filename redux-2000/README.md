...
Yet another attempt to fully wrap my brain around Redux.

## Aspects of Redux

- Store contains the 'state' (which is global state, not component state)
- State defines updates the UI/view with new data.
- UI/views trigger Actions, which are JS objects (the payload).
- Actions are sent to the Reducers which handle the payload and it's data
- Reducers are combined and used to update the Store.
- Rinse and repeat.
